...
export default function DesignerCommissionEdit({
  designerData,
  onSave,
  onClose,
}) {
  const [commission, setCommission] = useState(designerData.commission);

  const handleSubmit = (e) => {
    e.preventDefault();
    toast.success(`${designerData.name} Commission updated!`);
    onSave(designerData.id, { commission: Number(commission) });
    onClose();
  };
